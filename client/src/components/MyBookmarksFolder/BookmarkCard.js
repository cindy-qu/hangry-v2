...
// const newidhover = `#${idhover}`
  ...
  const linkAddEdit = personal_note?.personal_note.length > 0 ? `editNote/${note_id}` : `addNote/${bookmark_id}`
...
const closeToggle = (e) => {
  ...
  return (
    ...
    <div className="mt-3 mb-3 max-w-md bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 ">
      <div className="flex ">
        <h5 className="flex-1 text-center border-neutral-100 px-6 py-3 dark:border-neutral-600 dark:text-neutral-50 text-l font-bold tracking-tight text-gray-900 dark:text-white">{restaurant_name}</h5>
        <button onClick={handleToggle} className="mr-2"><i className="fa-solid fa-xmark"></i></button>
          ...
          <div className="fixed z-10 overflow-y-auto top-0 w-full left-0">
            <div className="flex items-center justify-center min-height-100vh pt-4 px-4 pb-20 text-center sm:block sm:p-0">
              ...
              <div className="inline-block align-center bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <h1 className="px-8 py-8 ">Are you sure you want to delete your bookmark for <span className="font-bold">{restaurant_name}</span>?</h1>
                <div className="bg-gray-200 px-4 py-3 text-center">
                  <button type="button" className="py-2 px-4 bg-gray-500 text-white rounded hover:bg-gray-700 mr-2" onClick={closeToggle} >Cancel</button>
                  <button type="button" className="py-2 px-4 bg-sky-700 text-white rounded hover:bg-sky-800 mr-2" onClick={handleDelete} >Delete</button>
                </div>
              </div>
            </div>
          </div>

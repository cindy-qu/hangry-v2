...
const AddBookmarkNote = ({ user, setUpdateBookmarkNote }) => {
  const [createPersonalNote, setCreatePersonalNote] = useState(
  ...
  return (
    <div className="text-center">
      ...
      <textarea
        ...
        onChange={(e) => {
          setCreatePersonalNote(e.target.value);
        }}
      />
      <br></br>
      <button
      ...
      <div id="edit-complete-msg" className={editMsgClassName}>
        <h3>Note Added!</h3>
        <Link to="/myBookmarks">
          <button className="ml-3 mr-3 px-3 py-1 rounded-md bg-sky-700 hover:bg-sky-800 text-white">
            View My Bookmarks
          </button>
        </Link>
      </div>

...
const Calendar = ({ user }) => {
  ...
  let matchRestaurantName = bookmarkId.find((book) =>
    book.id === paramsId ? book.id : "",
  );
    ...
    <div className="calendarpage">
      ...
      <form className="mb-3" onSubmit={handleSubmitEvent}>
        ...
        <select
          ...
          onChange={handleRestaurantTimezone}
          required
        >
          <option></option>
        ...
        <br></br>
        <button className="btn btn-primary" id="event-button" type="submit">
          Create Event
        </button>
        <div id="calendar-complete-msg" className={editCalendarEvent}>
          <h3>Event added!</h3>
          <Link to="/myBookmarks">
            <button className="btn btn-success">View My Bookmarks</button>
          </Link>
        </div>
      </form>
    </div>

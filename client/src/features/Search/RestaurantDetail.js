...
const RestaurantDetail = ({ user }) => {
  ...
  const modalClass = modal
    ...
    : "hidden fixed z-10 overflow-y-auto top-0 w-full left-0";
  ...
  const locationMap = locationAddress?.map((address) => {
    return (
      <p className="justify-center" key={address}>
        {address}
      </p>
    );
  });
  ...
  var newnumber = parseInt(
    restaurantDetail?.rating?.toString().replace(".", ""),
  );

  const ratingPlaceholder = newnumber ? newnumber : 0;

  const formErrorMsg = errors?.map((err) => <p key={err}>{err}</p>);

  const editMsgClassName = updated ? "" : "hidden";

  return (
    <div
      className=" bg-cover bg-bottom bg-no-repeat  h-[calc(100vh-64px)] overflow-auto w-full bg-[length:70rem] 
    sm:bg-[length:85rem]
    md:bg-[length:100rem] 
    lg:bg-[length:115rem] 
    xl:bg-[length:130rem] 
    ...
    >
      ...
      >
        <h2 className="text-center text-2xl pb-4 mt-2 sm:mt-4">
          {restaurantDetail?.name}
        </h2>
        <img
          className=" mx-auto rounded-md w-64 max-h-64 2xl:max-h-fit "
        ...
        <div className="text-center pt-4">
          <ul>
            ...
            <img
              className="mx-auto"
              src={`/images/small_${ratingPlaceholder}.png`}
              alt={restaurantDetail?.rating}
            ></img>
          </ul>
          <ul className="flex justify-center pt-3">
            Price Range: {restaurantDetail?.price}
          </ul>
          <ul className="justify-center">Address: {locationMap}</ul>
          <a href={restaurantDetail?.url} target="_blank" rel="noreferrer">
            <button className="mt-2 px-3 py-1 rounded-lg bg-sky-700 text-white">
              Yelp Page
            </button>
          </a>
          <br></br>
          <button
            ...
            className="mt-2  px-3 py-1 rounded-lg bg-sky-700 text-white"
            id="buttonbookmark"
          >
            Bookmark for Later
          </button>
          ...
          <div id="edit-complete-msg" className={editMsgClassName}>
            <h3>Bookmarked!</h3>
            <Link to="/myBookmarks">
              <button className="py-2 px-4 bg-sky-700 text-white rounded hover:bg-sky-800 mr-2">
                View My Bookmarks
              </button>
            </Link>
          </div>
